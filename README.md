...
- Business Suite (`business-suite`, `business`)
  - Commerce Platform (`commerce-platform`, `product-family`)
    - Commerce Core (`commerce-core`, `product`)
      ...
      - Commerce Inventory Service (`commerce-inventory`, `service`)
      - Commerce Order Service (`commerce-order`, `service`)
      - Commerce Pricing Service (`commerce-pricing`, `service`)
      - Commerce Tax Service (`commerce-tax`, `service`)
    - Fulfillment Hub (`fulfillment-hub`, `product`)
      - Fulfillment Picking Service (`fulfillment-picking`, `service`)
      ...
      - Fulfillment Tracking Service (`fulfillment-tracking`, `service`)
    - Merchant Insights (`merchant-insights`, `product`)
      - Insights Analytics Service (`insights-analytics`, `service`)
      - Insights Event Service (`insights-events`, `service`)
      - Insights Forecast Service (`insights-forecast`, `service`)
      - Insights Reporting Service (`insights-reporting`, `service`)
      - Insights Warehouse Service (`insights-warehouse`, `service`)
    - Payments Suite (`payments-suite`, `product`)
      ...
      - Payments Notifications Service (`payments-notifications`, `service`)
      - Payments Reconciliation Service (`payments-reconciliation`, `service`)
  ...
  - Customer Experience (`customer-experience`, `product-family`)
    - Customer Engagement (`customer-engagement`, `product`)
      ...
      - Engagement Preferences Service (`engagement-preferences`, `service`)
      - Engagement Profile Service (`engagement-profile`, `service`)
      - Engagement Segmentation Service (`engagement-segmentation`, `service`)
    - Loyalty Cloud (`loyalty-cloud`, `product`)
      - Loyalty Accounts Service (`loyalty-accounts`, `service`)
      - Loyalty Ledger Service (`loyalty-ledger`, `service`)
      - Loyalty Partner Service (`loyalty-partner`, `service`)
      - Loyalty Redemption Service (`loyalty-redemption`, `service`)
      - Loyalty Rewards Service (`loyalty-rewards`, `service`)
    - Marketing Automation (`marketing-automation`, `product`)
      - Marketing Attribution Service (`marketing-attribution`, `service`)
      - Marketing Content Service (`marketing-content`, `service`)
      - Marketing Insights Service (`marketing-insights`, `service`)
  ...
  - Operations Suite (`operations-suite`, `product-family`)
    - Support Desk (`support-desk`, `product`)
      ...
      - Support Knowledge Service (`support-knowledge`, `service`)
      - Support Routing Service (`support-routing`, `service`)
      - Support SLA Service (`support-sla`, `service`)
    - Supply Ops (`supply-ops`, `product`)
      - Supply Compliance Service (`supply-compliance`, `service`)
      - Supply Planning Service (`supply-planning`, `service`)
      - Supply Procurement Service (`supply-procurement`, `service`)
      - Supply Quality Service (`supply-quality`, `service`)
      - Supply Vendor Service (`supply-vendor`, `service`)
    - Workforce Hub (`workforce-hub`, `product`)
      - Workforce Access Service (`workforce-access`, `service`)
      - Workforce Payroll Service (`workforce-payroll`, `service`)
      - Workforce Scheduling Service (`workforce-scheduling`, `service`)
      - Workforce Timekeeping Service (`workforce-timekeeping`, `service`)
      - Workforce Training Service (`workforce-training`, `service`)
