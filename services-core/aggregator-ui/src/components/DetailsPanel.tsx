...
type DetailsPanelProps = {
    ...
    dependencyActorsByItemId: Record<string, { owner: CatalogActor | null; otherActors: CatalogActor[] } | null>;
    selectedItemActors: { owner: CatalogActor | null; otherActors: CatalogActor[] } | null;
    actorContactsByActorId: Map<string, { contacts: CatalogContact[]; primaryContact: CatalogContact | null }>;
...
export default function DetailsPanel({
    ...
    const ownerActorForContacts = selectedItemActors?.owner || selectedItemActors?.otherActors?.[0] || null;
    const otherActorsForContacts = selectedItemActors
        ? (selectedItemActors.owner ? selectedItemActors.otherActors : selectedItemActors.otherActors.slice(1))
        : [];
    const contactsActorsCount = (ownerActorForContacts ? 1 : 0) + otherActorsForContacts.length;
            ...
            <div className="contacts-actor-row" key={actor.id}>
                <div className="contacts-actor-grid">
                    <div className="contacts-actor-cell">
                        ...
                        >
                            <span className="contact-chip-text">{actor.title || actor.id}</span>
                            <span
                                className="contacts-actor-type-mark">{String(roleLabel || actor.type || 'other').toLowerCase()}</span>
                        </a>
                    </div>
                            ...
                            <>
                                ...
                                <ul className="actor-modal-contacts contacts-actor-contacts-list">
                                    {sortedContacts.map((contact) => {
                                        ...
                                        return (
                                            <li key={contact.id}>
                                                {renderContactChip(contact, {isPrimary: isPrimaryContact})}
                                            </li>
                                        );
                                    })}
                                </ul>
    ...
    const renderActorAndContactCells = (ownerActor: CatalogActor | null) => {
        ...
        const primaryContact = actorContactsByActorId.get(ownerActor.id)?.primaryContact || null;
                ...
                <div className="actor-inline-info">
                    <span className="actor-inline-pair">
                        ...
                        <a
                            ...
                            title={ownerLabel}
                        >
                            <span className="contact-chip-text inline-link-text">{ownerLabel}</span>
                        </a>
                    </span>
                    {primaryContact && (
                        <span className="actor-inline-pair">
                            ...
                            >
                                <span
                                    ...
                                    aria-label={resolveContactTypeDisplayName(primaryContact.type)}
                                >
                                    <svg
                                        ...
                                        aria-hidden="true"
                                    >
                                        <use href={`${iconSpriteHref}#${resolveContactIconId(primaryContact.type)}`}/>
                                    </svg>
                                </span>
                                <span className="contact-chip-text inline-link-text">{primaryContactLabel}</span>
                            </a>
                        </span>
                    )}
                </div>
                ...
                <>
                            ...
                            <div className="details-panel-body details-panel-body-signals">
                                <div className="contacts-actors-block">
                                    {ownerActorForContacts ? (
                                        renderActorContactsRow(ownerActorForContacts, selectedItemActors?.owner ? 'owner' : ownerActorForContacts.type)
                                    ) : (
                                        <div className="contacts-actor-row">
                                            <span
                                                className="dependency-actor-empty">No actors linked to this item</span>
                                        </div>
                                    )}
                                </div>
                                {otherActorsForContacts.length > 0 && (
                                    <section className={`signals-subsection ${isContactsOtherOpen ? 'is-open' : ''}`}>
                                        <button
                                            type="button"
                                        ...
                                        >
                                            ...
                                            <span className="signals-subtitle">
                                                ...
                                                {' '}
                                                <span
                                                    className="details-panel-count">({otherActorsForContacts.length})</span>
                                            </span>
                                        </button>
                                        {isContactsOtherOpen && (
                                            <div className="contacts-actors-block">
                                                {otherActorsForContacts.map((actor) => renderActorContactsRow(actor))}
                                            </div>
                                        )}
                                    </section>
                                )}
                    ...
                    >
                        ...
                        {isSignalsOpen && (
                            <div className="details-panel-body details-panel-body-signals">
                                {hasOwnFailingSignals && (
                                    <section className={`signals-subsection ${isFailingSignalsOpen ? 'is-open' : ''}`}>
                                        <button
                                            type="button"
                                        ...
                                        >
                                            ...
                                            Failing
                                                {' '}
                                                <span
                                                    className="details-panel-count">({selectedFailingSignals.length})</span>
                                        </span>
                                        </button>
                                        {isFailingSignalsOpen && (
                                                    ...
                                                    <div className="dependency-main">
                                                        <ul className="own-failing-signals-list">
                                                            {selectedFailingSignals.map((entry) => (
                                                                <li key={entry.id} className="signal own-failing-signal">
                                    ...
                                    <section
                                        ...
                                        >
                                            ...
                                            Affected by
                                                {' '}
                                                <span
                                                    className="details-panel-count">({failingDependencies.length})</span>
                                        </span>
                                        </button>
                                        {isAffectedBySignalsOpen && (
                                            <ul className="signals-list signals-sublist">
                                                {failingDependencies.map((entry) => {
                                                    const dependencyActors = dependencyActorsByItemId[entry.id];
                                                    return (
                                                        <li key={entry.id} className="dependency dependency-with-actor">
                                ...
                                <section className={`signals-subsection ${isPassingSignalsOpen ? 'is-open' : ''}`}>
                                    <button
                                        type="button"
