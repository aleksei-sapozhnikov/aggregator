  ...
  const row = (
    ...
    >
      {hasChildren ? (
        ...
        <span
          className="node-toggle-column node-toggle-spacer"
          aria-hidden="true"
        />
      )}
      <a
        ...
        <div className="node-main">
          <div className="node-label">
            <span className="node-heading">
              ...
              />
              {node.item.title && (
                <span className="node-name">{node.item.title}</span>
              )}
            </span>
          </div>
        </div>
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-header">
        <span className="sidebar-header-title" title={sidebarTitle}>
          {sidebarTitle}
        </span>
      </div>
        ...
        <div className="tree-search">
          ...
          <datalist id={searchSuggestionsListId}>
            {searchAutocompleteOptions.map((option) => (
              <option
                key={option.fullQuery}
                value={option.fullQuery}
                label={option.word}
              />
            ))}
          </datalist>
      ...
      {!error && tree.length > 0 && (
        <div className="tree-controls">
          ...
          >
            <span className="tree-control-icon" aria-hidden="true">
              ⇧
            </span>
      ...
      {!error && tree.length === 0 && (
        <div className="empty">
          Catalog is empty. Add items in catalog service configuration.
        </div>
      )}
      {isSearchActive ? (
