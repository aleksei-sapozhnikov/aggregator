...
public class CatalogValidator {
	...
	public Catalog validate(@Nonnull CatalogDefinition definition) {
		...
		List<CatalogDefinition.ItemDefinition> itemDefinitions = requireList(
			definition.items(),
			"items"
		);
		List<CatalogDefinition.DependencyDefinition> dependencyDefinitions = requireList(
	...
	private List<Dependency> parseDependencies(
		...
		for (int index = 0; index < dependencyDefinitions.size(); index++) {
			...
			if (definition == null) {
				throw new IllegalStateException("Catalog dependency at index " + index + " must not be null");
			}
			String sourceValue = requireText(definition.sourceId(), "sourceId", "dependency at index " + index);
			String targetValue = requireText(definition.targetId(), "targetId", "dependency at index " + index);
			String type = requireText(definition.type(), "type", "dependency " + sourceValue + "->" + targetValue);
			ItemId sourceId = ItemId.of(sourceValue);
			ItemId targetId = ItemId.of(targetValue);
			if (!knownItemIds.contains(sourceId)) {
				throw new IllegalStateException("Dependency sourceId not found in catalog items: " + sourceId);
			}
			if (!knownItemIds.contains(targetId)) {
				throw new IllegalStateException("Dependency targetId not found in catalog items: " + targetId);
			}
